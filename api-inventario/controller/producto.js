...
function getProductos(req, res) {
    if (con) {
        con.query(`SELECT * FROM producto ORDER BY referencia ASC`, function(err, result) {
            if (err) {
...
function saveProducto(req, res) {
    if (con) {
        ...
        con.query(`INSERT INTO producto ( referencia, tamano, precioUnit ) VALUES ( '${params.referencia}',
     '${params.tamano}', '${params.precioUnit}')`, function(err, result) {
            if (err) {
...
function updateProducto(req, res) {
    if (con) {
        ...
        const sql = 'UPDATE producto SET  referencia = ' + con.escape(update.referencia) +
            ', tamano = ' + con.escape(update.tamano) + ', precioUnit = ' + con.escape(update.precioUnit) +
            ' WHERE idproducto = ' + id;
